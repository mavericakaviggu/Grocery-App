...
function callApi(method, url, data) {
  // Code block uses jQuery's $.ajax() function
  //The .done() method is a jQuery promise method that specifies a callback function to be executed when the AJAX request is successfully completed.
  //The callback function reloads the current page (window.location.reload()) after the AJAX request is done.
  //The msg parameter in the callback function represents the response returned by the server, but it is not used in this code snippet.
  $.ajax({
    method: method,
    url: url,
    data: data,
